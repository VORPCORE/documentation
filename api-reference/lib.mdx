    ...
    <Tab title="Client Getter">
        ...
        <Accordion icon="code" iconType="duotone" title="GetDensityMultipliers">
            ...
            <ParamField body="return" type="table">
                Returns either one density entry or the full density table. A single entry contains `enable`, `value`, and optionally `temp_value`.
            </ParamField>
            ...
            print(vehicleDensity.enable, vehicleDensity.value, vehicleDensity.temp_value)
            ```
        </Accordion>
    </Tab>
    ...
    <Tab title="Server Setters">
        ...
        <Accordion icon="code" iconType="duotone" title="SetDefaultDensityMultipliers">
            ...
            <ParamField body="value" type="number" required>
                Density multiplier value. The source code uses values between `0.0` and `1.0`.
            </ParamField>

            <ParamField body="enable" type="boolean" required>
                Must be `true` for the change to be applied. Passing `false` does not reset the default value with the current implementation.
            </ParamField>

            ```lua
            local target <const> = source
            exports.vorp_lib:SetDefaultDensityMultipliers(target, "VehicleDensity", 0.2, true)
            exports.vorp_lib:SetDefaultDensityMultipliers(target, "PedDensity", 0.0, true)
            ```
        </Accordion>
        ...
        <Accordion icon="code" iconType="duotone" title="SetTemporaryDensityMultipliers">
            ...
            <ParamField body="value" type="number" required>
                Temporary density multiplier value.
            </ParamField>

            <ParamField body="enable" type="boolean" required>
                Must be `true` for the temporary override to be applied. Passing `false` does not remove the temporary value with the current implementation.
            </ParamField>

            ```lua
            local target <const> = source
            exports.vorp_lib:SetTemporaryDensityMultipliers(target, "ScenarioHumanDensity", 0.0, true)
            exports.vorp_lib:SetTemporaryDensityMultipliers(target, "RandomVehicleDensity", 0.1, true)
            ```
        </Accordion>
