...
<Warning>
    to import the notify module on the server you must also import on the client side, events are registered on the client side so that we can use server side notifications. to just import on the client  no need to import on the server.
</Warning>
